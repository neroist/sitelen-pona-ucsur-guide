...
BEGIN_TABLE
"""
    table += "\n".join(f"{to_short(i[0])[0] if short else i[0]}\t{i[1]}\t1" for i in sorted(words, key=sorted_key, reverse=True))
    table += """
[	󱦐	1
]	󱦑	1
*	‍	1
...
"""
    return header.lstrip() + "\n".join(["::" + (to_short(i[0])[0] if short else i[0]) + "::{U+" + str(hex(ord(i[1])))[2:].upper() + "} ; " + i[1]
...
to 」
< 「
> 」""" + "\n".join((f"{to_short(i[0])[0] if short else i[0]} {i[1]}" for i in words)) + "\nENDCHARACTER"
